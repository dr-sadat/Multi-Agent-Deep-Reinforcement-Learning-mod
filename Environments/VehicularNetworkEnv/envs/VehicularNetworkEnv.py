...
class VehicularNetworkEnv(gym.Env):
    ...
    def __init__(self, experiment_config: ExperimentConfig, trajectories_file_name):
        ...
        #print('New Comm ', self.range)
        
        

        self.experiment_config = experiment_config
        assert self.experiment_config is not None
        #self.experiment_config.range = range #SADAT
        #self.experiment_config.communication_range = 100 #SADAT
    ...
    def init_experiences_global_trajectory(self):
        ...
        for vehicle_id in random_vehicle_id:
            ...
            for row in new_df.itertuples():
                ...
                if ((y - self.experiment_config.edge_node_y) == 0):
                	print('y: ', y)
                	print(self.experiment_config.edge_node_y)
    ...
    def step_with_difference_rewards(self, action: dict):
        ...
        for edge_view_index in range(self.experiment_config.edge_views_number):
            if self.edge_views_in_edge_node[edge_view_index][self.episode_step] == 1:
                ...
                # new_age_of_view = 6 / 10 * (1 - np.tanh(new_age_timeliness / 100)) + 2 / 10 * (1 - np.tanh(new_age_consistence / 1000)) + 2 / 10 * completeness   #SADAT             

                age_of_view = 3 / 10 * (1 - np.tanh(new_age_timeliness / 100)) + 1 / 10 * (1 - np.tanh(new_age_consistence / 1000)) + 6 / 10 * completeness
                # age_of_view = 6 / 10 * (1 - np.tanh(new_age_timeliness / 100)) + 1 / 10 * (1 - np.tanh(new_age_consistence / 1000)) + 3 / 10 * completeness  #SADAT
        ...
        for difference_rewards_vehicle_index in range(self.experiment_config.vehicle_number):
            ...
            for edge_view_index in range(self.experiment_config.edge_views_number):
                if self.edge_views_in_edge_node[edge_view_index][self.episode_step] == 1:
                    ...
                    # difference_rewards_new_age_of_view = 6 / 10 * (1 - np.tanh(difference_rewards_new_age_timeliness / 100)) + 2 / 10 * (1 - np.tanh(difference_rewards_new_age_consistence / 1000)) + 2 / 10 * difference_rewards_completeness   #SADAT
                    ...
                    # difference_rewards_age_of_view = 6 / 10 * (1 - np.tanh(difference_rewards_new_age_timeliness / 100)) + 1 / 10 * (1 - np.tanh(difference_rewards_new_age_consistence / 1000)) + 3 / 10 * difference_rewards_completeness   #SADAT
    ...
    def update_sensor_observation(self):
        ...
        """
        #self.experiment_config.communication_range = 100 #SADAT
        # print('Comm range 2:', self.experiment_config.communication_range) #SADAT
        for vehicle_index in range(self.experiment_config.vehicle_number):
            index_start = 0
    ...
    def update_edge_observation(self):
        ...
        for vehicle_index in range(self.experiment_config.vehicle_number):
            for data_type_index in range(self.experiment_config.data_types_number):
                ...
                index_start += 1
        #print('Comm range:', self.experiment_config.communication_range) #SADAT
        #print('New Comm ', self.range) #SADAT
    ...
    def compute_SNR(self, vehicle_index, time_slot):
        ...
        :return: SNR ratio
        """
        self.experiment_config.communication_range = 500 #SADAT Ashol jayga ***
    ...
    def compute_SNR_by_distance(self, distance):
        ...
        SNR = (1 / white_gaussian_noise) * np.power(np.abs(channel_fading_gain), 2) * \
            1 / (np.power(distance, self.experiment_config.path_loss_exponent)) * \
            VehicularNetworkEnv.cover_mW_to_W(self.experiment_config.transmission_power)
        # print('Comm range 3:', self.experiment_config.communication_range) #SADAT
        return SNR
            ...
            for data_type_index in range(self.experiment_config.data_types_number):
                if self.data_types_in_vehicles[vehicle_index][data_type_index] == 1:
                    ...
                    for time_slot_index in time_slot_index_list:
                        ...
                        if (distance!=0):
                            channel_fading_gain = self.experiment_config.mean_channel_fading_gain
                            # print(np.power(distance, self.experiment_config.path_loss_exponent))
                            # print(distance)
                        	#print(self.experiment_config.path_loss_exponent)
